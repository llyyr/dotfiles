...
# Type: Dict
c.aliases = {'w': 'session-save', 'wq': 'quit --save', 'anime': 'open -t http://anichart.net', 'yt': 'open -t https://www.youtube.com/feed/subscriptions', 'otc': 'open -t {clipboard}', 'mpv': 'spawn -d mpv --profile=web {url}', 'mpa': 'spawn -d mpv --profile=audio {url}'}
...
allowed_domains = ['*.nyaa.si/*', '*.github.com/*', '192.168.1.1', '*.youtube.com/*', '*.reddit.com/*', '*.twitch.tv/*', '*.4channel.org/*', 
    '*://osu.ppy.sh/*', '*.cricbuzz.com/*', '*.pixiv.net/*', '*.searx.me/*', 'file://*', 'chrome://*/*', 'qute://*/*']
...
config.bind(';M', 'hint links spawn -d streamlink {hint-url} best')
config.bind(';T', 'open -t https://translate.google.com/#auto/en/{url}')
config.bind('cc', 'config-cycle -t content.user_stylesheets "css/global.css" ""')
